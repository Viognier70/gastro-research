...
import { mergeKeywordsCaseInsensitive } from '../_shared/openalex-terms.ts'
import { runSci, ROLES } from '../_shared/haiku-sci.ts'
...
async function save(article: any, sci: any, triad: ReturnType<typeof parseLabeledProse> | null) {
  try {
    ...
    if(sci) {
      ...
      // _shared/openalex-terms.ts. Löser samtidigt buggen där tidigare
  ...
  for(const item of batch) {
    ...
    try {
      ...
      if (!item.sci_done && article.abstract && article.abstract.length > 50) {
        sciCalls++
        sci = await runSci(article, ANTHROPIC_KEY)
      }
